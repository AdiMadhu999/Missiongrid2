...
export const MentorLoginForm: React.FC<MentorLoginFormProps> = ({ onSuccess, error, setError, loading, setLoading }) => {
  ...
  const [loginCooldown, setLoginCooldown] = useState(0); // Added loginCooldown
  ...
  React.useEffect(() => { // Added loginCooldown effect
    if (loginCooldown > 0) {
  ...
  return (
    <form className="space-y-4" onSubmit={handleLogin}>
      ...
      >
        {loginCooldown > 0 ? `Wait ${loginCooldown}s` : (loading ? 'Processing...' : (showOtpInput ? (loginMethod === 'sms' ? 'Verify OTP' : 'Verify PIN') : 'Access Portal'))}
      </button>
    </form>
  );
};
