          ...
          {expanded && (
            ...
            >
              <div className="p-6 space-y-6">
                ...
                {/* 3-Approach Solution Display */}
                <div className="space-y-6">
                  {/* Method 1: Step-by-Step Detailed Solution */}
                  <div className="space-y-2 p-4 bg-indigo-50/40 rounded-2xl border border-indigo-100">
                    <div className="flex items-center justify-between">
                      <h4 className="flex items-center gap-2 text-indigo-950 font-extrabold text-xs uppercase tracking-wider">
                    ...
                    <div className="space-y-4 pt-2">
                      {question.stepwiseSolution?.map((step, sIdx) => (
                        <div key={`${question.id}-step-${sIdx}`} className="flex gap-4">
                          ...
                          <div className="pb-2 text-slate-700 leading-relaxed text-sm">
                  ...
                  {/* Method 2: Exam Approach & Shortcut Trick */}
                  {question.examApproach && (
                    <div className="space-y-2 p-4 bg-emerald-50/50 rounded-2xl border border-emerald-100">
                      <div className="flex items-center justify-between">
                        <h4 className="flex items-center gap-2 text-emerald-950 font-extrabold text-xs uppercase tracking-wider">
                          <Calculator className="w-4 h-4 text-emerald-600" />
                          Method 2: Exam Approach & Rapid Trick
                        </h4>
                        <span className="text-[9px] font-black uppercase px-2 py-0.5 bg-emerald-600 text-white rounded-full">
                          Fast Exam Shortcut
                        </span>
                      </div>
                      <div className="text-slate-700 leading-relaxed text-sm pt-1">
                        <MathRenderer content={question.examApproach} />
                      </div>
                    </div>
                  )}

                  {/* Method 3: Core Theorem & Alternative Method */}
                  {question.ruleOrTheorem && (
                    <div className="space-y-2 p-4 bg-blue-50/50 rounded-2xl border border-blue-100">
                      <div className="flex items-center justify-between">
                        <h4 className="flex items-center gap-2 text-blue-950 font-extrabold text-xs uppercase tracking-wider">
                          <Bookmark className="w-4 h-4 text-blue-600" />
                          Method 3: Core Theorem & Alternative Method
                        </h4>
                        <span className="text-[9px] font-black uppercase px-2 py-0.5 bg-blue-600 text-white rounded-full">
                          Formula & Concept
                        </span>
                      </div>
                      <div className="text-slate-700 leading-relaxed text-sm pt-1">
