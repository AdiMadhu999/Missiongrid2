...
export function checkPremiumStatus(profile: any): { isPremium: boolean; premiumStatus: string; remainingPremiumDays: number } {
  return { isPremium: true, premiumStatus: 'PREMIUM', remainingPremiumDays: 9999 };
}

export const usePremiumToggle = () => {
  const { togglePremium, userProfile } = useAuth();
  return {
...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const claimsSyncInProgressRef = useRef<boolean>(false);

  const togglePremium = React.useCallback(() => {
    setPremiumOverride(prev => {
      const activeDetails = userProfileRef.current ? checkPremiumStatus(userProfileRef.current) : { isPremium: false };
      const currentVal = prev !== null ? prev : activeDetails.isPremium;
      return !currentVal;
    });
  }, []);
  ...
  const handleSetUserProfile = React.useCallback((profile: User | null) => {
    ...
    if (
      ...
      current?.isPremium === profile?.isPremium &&
      current?.premiumStatus === profile?.premiumStatus &&
    ...
    const unsubscribeAuth = onAuthStateChanged(auth, async (user) => {
      ...
      if (user) {
        ...
        if (cachedProfileData && cachedProfileData.id && (claims.role || user.isAnonymous)) {
          const claimsPremium = (claims.premiumStatus as string);
          const hasActiveClaimsPremium = claimsPremium === 'PREMIUM' || claimsPremium === 'active';
          ...
          const premiumDetails = checkPremiumStatus(combined);
          combined.isPremium = claimsPremium ? hasActiveClaimsPremium : premiumDetails.isPremium;
          combined.premiumStatus = claimsPremium ? claimsPremium : premiumDetails.premiumStatus;
          combined.remainingPremiumDays = premiumDetails.remainingPremiumDays;
        ...
        const handleCombinedProfile = (pubData: any, privData: any, userId: string) => {
          ...
          // Centralized Real-time Premium Trial & Subscription Management
          const premiumDetails = checkPremiumStatus(combinedProfile);
          const isMentor = ['mentor', 'primary-mentor', 'primarymentor', 'staff', 'admin', 'examiner'].includes((pubData.role || '').toLowerCase());
          const calculatedPremiumStatus = premiumDetails.premiumStatus;

          combinedProfile.isPremium = premiumDetails.isPremium;
          combinedProfile.premiumStatus = premiumDetails.premiumStatus;
          combinedProfile.remainingPremiumDays = premiumDetails.remainingPremiumDays;
          combinedProfile.testAccess = premiumDetails.isPremium ? 'premium' : 'free';

          // If the student's premium has expired (completed), update the database to revert their status back to FREE
          const dbSaysPremium = !!pubData.isPremium || pubData.premiumStatus === 'PREMIUM' || pubData.premiumStatus === 'active';
          if (!isMentor && dbSaysPremium && !premiumDetails.isPremium) {
            console.log(`[Premium Expiry] Student ${userId} premium duration completed. Auto-converting to FREE in Firestore.`);
            ...
            }).catch(e => console.warn("[Premium Expiry] Failed to update expired user to FREE in Firestore", e));
          }
          ...
          // Instantly sync custom claims on token when Firestore premium status changes
          if (!isCurrentAnonymous) {
            const claimsPremiumStatus = claims.premiumStatus || 'FREE';
            const firestorePremiumStatus = calculatedPremiumStatus || 'FREE';
            
            if (claimsPremiumStatus !== firestorePremiumStatus) {
              if (claimsSyncInProgressRef.current) {
                console.log("[Premium Sync] Claims sync already in progress. Skipping parallel sync.");
              } else {
                claimsSyncInProgressRef.current = true;
                console.log(`[Premium Sync] Premium status mismatch detected: Claims is ${claimsPremiumStatus}, Firestore is ${firestorePremiumStatus}. Syncing...`);
                user.getIdToken().then(idToken => {
                  apiFetch('/api/auth/sync-claims', {
                    method: 'POST',
                    headers: {
                    ...
                    if (res.ok) {
                      console.log("[Premium Sync] Custom claims successfully synced on backend. Force refreshing token...");
                      const freshTokenResult = await user.getIdTokenResult(true);
                      claims = freshTokenResult.claims;
                      console.log("[Premium Sync] ID Token refreshed successfully with updated claims:", claims);
                    } else {
                      console.warn("[Premium Sync] Backend claim sync returned non-ok response status.");
                    }
                  }).catch(err => {
                    console.warn("[Premium Sync] Background sync claims error:", err);
                  }).finally(() => {
                    claimsSyncInProgressRef.current = false;
                  });
                });
              }
            }
  ...
  const authContextValue = React.useMemo(() => {
    ...
    if (resolvedProfile) {
      const premiumDetails = checkPremiumStatus(resolvedProfile);
      resolvedProfile = {
        ...resolvedProfile,
        isPremium: premiumOverride !== null ? premiumOverride : premiumDetails.isPremium,
        premiumStatus: premiumOverride !== null ? (premiumOverride ? 'PREMIUM' : 'FREE') : premiumDetails.premiumStatus,
        remainingPremiumDays: premiumOverride !== null ? (premiumOverride ? 9999 : 0) : premiumDetails.remainingPremiumDays
      };
    }
    return {
      currentUser,
  ...
  }, [currentUser, userProfile, loading, logout, handleSetUserProfile, premiumOverride, togglePremium]);
