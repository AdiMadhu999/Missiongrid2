...
import { Search, Plus, Megaphone, FileText, Mic, Video, HelpCircle, Image } from 'lucide-react';
...
import { MentorPostCard, DoubtCard, DailyTestCard, ArticleCard, VoiceNoteCard, PollCard } from '../components/feed/FeedCards';
...
export default function MissionFeedScreen({ feedType = 'all' }: MissionFeedScreenProps) {
  ...
  const [activeTab, setActiveTab] = useState<'Mentor Message' | 'Video Guidance' | 'Study Material'>('Mentor Message');
    ...
    subscribeFn: (callback) => {
      ...
      return onSnapshot(qDoubts, (snap) => {
        ...
        const filteredDoubts = allDoubts.filter(doubt => {
          ...
          const isAuthor = doubt.authorId === userProfile?.id;
          const isSameBatch = doubt.batchId === userProfile?.batchId ||
              (doubt.batchIds && Array.isArray(doubt.batchIds) && doubt.batchIds.includes(userProfile?.batchId));
          return isAuthor || isSameBatch;
        });
  ...
  const processedItems = React.useMemo(() => {
    let baseItems: any[] = [];
    const studentDay = userProfile?.registrationDate ? calculatePreparationDay(userProfile.registrationDate) : 1;
    ...
    const filtered = baseItems.filter(item => {
        ...
        if (highlightId && item.id === highlightId) return true;

        // Visibility and Tab Filtering
        if (feedType === 'guide' && item.type === 'MentorPost') {
           const post = item as MentorPost;
           if (post.dayNumber && post.dayNumber > studentDay) return false;
           
           if (post.category) {
                if (post.category !== activeTab) return false;
           } else {
                // Fallback for old posts
                if (activeTab === 'Mentor Message' && !['announcement', 'voiceNote', 'poll'].includes(post.postType)) return false;
                if (activeTab === 'Video Guidance' && post.postType !== 'video') return false;
                if (activeTab === 'Study Material' && !['article', 'pdf', 'image'].includes(post.postType)) return false;
           }
        }
        ...
        if (filterType === 'Polls') matchesFilter = item.type === 'MentorPost' && (item as MentorPost).postType === 'poll';
        ...
        if (filterType === 'PDF Documents') matchesFilter = item.type === 'MentorPost' && (item as MentorPost).postType === 'pdf';
        if (filterType === 'Images') matchesFilter = item.type === 'MentorPost' && (item as MentorPost).postType === 'image';
        if (filterType === 'Doubts') matchesFilter = item.type === 'Doubt';
        if (filterType === 'Private Doubts') matchesFilter = item.type === 'Doubt' && (item as any).privacy === 'private';
        if (filterType === 'My Public Doubts') matchesFilter = item.type === 'Doubt' && (item as any).privacy === 'public' && (item as any).authorId === userProfile?.id;
        if (filterType === 'Replied Doubts') matchesFilter = item.type === 'Doubt' && (item as any).status === 'Solved';
        if (filterType === 'Unreplied Doubts') matchesFilter = item.type === 'Doubt' && (item as any).status === 'Unsolved';
    ...
    return sorted.map(item => {
        if (item.type === 'Doubt' && item.privacy === 'private') {
             ...
             const isAuthorized = item.authorId === userProfile?.id || item.mentorId === userProfile?.id || userProfile?.role === 'mentor' || isSharedWithMe;
             if (!isAuthorized) {
                 privateDoubtCount++;
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      ...
      <header className="sticky top-0 z-50 bg-white border-b border-slate-100 shadow-sm">
        ...
        {feedType !== 'guide' && (
          <div className="flex gap-2 px-4 py-3 overflow-x-auto scrollbar-hide border-t border-slate-50 bg-white">
            {((): string[] => {
                let options = ['All'];
                if (userProfile?.role === 'mentor') {
                    options.push('Private Doubts', 'Replied Doubts', 'Unreplied Doubts');
                } else {
                    options.push('Private Doubts', 'My Public Doubts');
                }
                
                if (feedType !== 'doubt') {
                    options.push('Announcements', 'Articles', 'Polls', 'Tests', 'PDF Documents', 'Images', 'Voice Notes', 'Videos', 'Bookmarks');
                }
                return options;
            })().map(type => {
        ...
        {feedType === 'guide' && (
            <div className="flex gap-2 px-4 py-3 overflow-x-auto scrollbar-hide bg-white border-t border-slate-50">
                {(['Mentor Message', 'Video Guidance', 'Study Material'] as const).map(tab => (
                    <button
                        key={tab}
                    ...
                    if (item.type === 'MentorPost') {
                        ...
                        card = (
                            <div className="py-2.5 px-4 mb-2 text-slate-500 text-xs font-bold select-none border-b border-slate-150/45">
                                • Mentor guide count {item.placeholderNumber}
                            </div>
                        );
          ...
          <div className="bg-white p-2 rounded-2xl shadow-xl border border-slate-100 flex flex-col gap-1 w-48 animate-in fade-in zoom-in duration-200">
            {userProfile?.role === 'mentor' && feedType !== 'doubt' && (
              <>
                ...
                <button onClick={() => { setSelectedPostType('poll'); setModalOpen(true); setIsOpen(false); }} className="flex items-center gap-3 p-3 hover:bg-slate-50 rounded-xl text-sm font-bold text-slate-900">📊 Poll</button>
                ...
                <button onClick={() => { setSelectedPostType('image'); setModalOpen(true); setIsOpen(false); }} className="flex items-center gap-3 p-3 hover:bg-slate-50 rounded-xl text-sm font-bold text-slate-900"><Image className="w-5 h-5 text-indigo-600" /> Image</button>
              </>
            )}
            {feedType !== 'guide' && (
