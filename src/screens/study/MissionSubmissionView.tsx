...
export default function MissionHub({ onBack }: Props) {
  ...
  const isSectionSubmissionBlocked = (sectionId: string): boolean => {
    let lastSubStr = userProfile?.lastSectionSubmissionAt?.[sectionId];
    
    // Check if submitted in today's report in real-time
    if (!lastSubStr) {
      const sectionData = todayReport?.sections?.[sectionId];
      if (sectionData?.submitted && (sectionData.status === 'Approved' || sectionData.status === 'Pending' || sectionData.status === 'Submitted')) {
        lastSubStr = sectionData.submittedAt;
      }
    }

    if (!lastSubStr) return false;
    const lastSub = new Date(lastSubStr).getTime();
    const now = Date.now();
    return now - lastSub < 24 * 60 * 60 * 1000;
  };

  const getSectionTimeRemainingText = (sectionId: string): string => {
    let lastSubStr = userProfile?.lastSectionSubmissionAt?.[sectionId];
    ...
    if (!lastSubStr) return '';
    const lastSub = new Date(lastSubStr).getTime();
    const now = Date.now();
    const diff = 24 * 60 * 60 * 1000 - (now - lastSub);
    if (diff <= 0) return '';
  ...
  const blockedSectionNames = SECTIONS_DEF
    .filter(sec => isSectionSubmissionBlocked(sec.id))
    .map(sec => sec.title.replace(' Submission', ''));
  ...
  const handleFileChange = async (sectionId: string, e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const compressionToastId = toast.loading('Optimizing files for upload...');
    ...
    try {
      for (const file of selectedFiles) {
        ...
        const isFinalImage = finalFile.type.startsWith('image/');
        validFiles.push({
          file: finalFile,
          preview: isFinalImage ? URL.createObjectURL(finalFile) : '',
          type: isFinalImage ? 'image' : 'pdf'
        });
      }
      ...
      toast.success('Files processed and ready!', { id: compressionToastId });
    } catch (err) {
      console.error(err);
      toast.error('Could not process some files.', { id: compressionToastId });
    } finally {
      // Clear the input value so the change handler triggers even if selecting the same/additional files again
      e.target.value = '';
    }
  };
  ...
  const isSubmissionLockedByTime = () => {
    // If there is an approved report in history
    const approvedReports = history.filter(r => r.status === 'Approved');
    if (approvedReports.length === 0) return false;

    // Find the latest approved report date
    const sortedApproved = [...approvedReports].sort((a, b) => b.date.localeCompare(a.date));
    const latestApprovedDateStr = sortedApproved[0].date;

    const todayDate = new Date();
    const currentHour = todayDate.getHours();

    // Check if tomorrow of latest approved date is the day we are trying to submit (today)
    try {
      const latestApprovedDate = new Date(latestApprovedDateStr + "T00:00:00");
      const nextDayOfApproved = new Date(latestApprovedDate.getTime() + 24 * 60 * 60 * 1000);
      const nextDayStr = nextDayOfApproved.toISOString().split('T')[0];

      if (today === nextDayStr && currentHour < 9) {
        return true;
      }
    } catch (e) {
      console.error("Error checking submission lock time:", e);
    }
    return false;
  };
        ...
        <AnimatePresence mode="wait">
          {activeTab === 'submit' ? (
            ...
            >
              ...
              {/* Submission Locked warning card */}
              {blockedSectionNames.length > 0 && (
                <div className="bg-gradient-to-r from-amber-500/10 to-orange-500/10 border-2 border-amber-200/50 rounded-3xl p-6 shadow-xs relative overflow-hidden flex items-start gap-4">
                  ...
                  <div className="space-y-1.5 flex-1">
                    <h3 className="text-sm font-black text-amber-950 tracking-tight flex items-center gap-1.5">
                      Category Submission Locked (24h Buffer)
                    </h3>
                    <p className="text-xs font-semibold text-amber-900 leading-relaxed">
                      To prevent duplicate submissions, each category of the mission can only be submitted once every 24 hours. The following categories are currently locked:
                    </p>
                    <div className="mt-2.5 flex flex-wrap gap-2">
                      {SECTIONS_DEF.filter(sec => isSectionSubmissionBlocked(sec.id)).map(sec => (
                        <div key={sec.id} className="bg-white/95 px-3 py-1.5 rounded-xl border border-amber-200/50 text-xs flex items-center gap-1.5 shadow-2xs">
                          <span className="font-bold text-slate-700">{sec.title.replace(' Submission', '')}:</span>
                          <span className="text-orange-600 font-black">{getSectionTimeRemainingText(sec.id)}</span>
                        </div>
                      ))}
                    </div>
                  </div>
              ...
              {/* Overall Status & Mentor Evaluation Remarks Card */}
              {todayReport && (
                <div className={`p-6 border-2 rounded-3xl relative overflow-hidden shadow-sm transition-all animate-in fade-in slide-in-from-top-4 duration-500 ${
                  todayReport.status === 'Approved' ? 'bg-gradient-to-r from-emerald-50/70 via-teal-50/40 to-emerald-50/70 border-emerald-200' :
                  todayReport.status === 'Needs Improvement' || todayReport.status === 'Resubmit Required' ? 'bg-gradient-to-r from-indigo-50/70 via-purple-50/40 to-indigo-50/70 border-indigo-200' :
                  todayReport.status === 'Rejected' ? 'bg-gradient-to-r from-rose-50/70 via-orange-50/40 to-rose-50/70 border-rose-200' :
                  'bg-gradient-to-r from-amber-50/70 via-orange-50/40 to-amber-50/70 border-amber-200'
                }`}>
                  <div className="flex items-start gap-4">
                    <div className={`w-10 h-10 rounded-xl flex items-center justify-center text-white shrink-0 shadow-md ${
                      todayReport.status === 'Approved' ? 'bg-emerald-500' :
                      todayReport.status === 'Needs Improvement' || todayReport.status === 'Resubmit Required' ? 'bg-indigo-500' :
                      todayReport.status === 'Rejected' ? 'bg-rose-500' :
                      'bg-amber-500'
                    }`}>
                      {todayReport.status === 'Approved' ? <CheckCircle2 size={20} /> :
                       todayReport.status === 'Needs Improvement' || todayReport.status === 'Resubmit Required' ? <HelpCircle size={20} /> :
                       todayReport.status === 'Rejected' ? <AlertCircle size={20} /> :
                       <Clock size={20} className="animate-spin" />}
                    </div>
                    <div className="space-y-2 flex-1">
                      <div className="flex justify-between items-center flex-wrap gap-2">
                        <h3 className="text-sm font-black text-slate-900 tracking-tight uppercase">
                          {todayReport.status === 'Approved' ? 'Mission Approved' :
                           todayReport.status === 'Needs Improvement' || todayReport.status === 'Resubmit Required' ? 'Needs Improvement' :
                           todayReport.status === 'Rejected' ? 'Submission Rejected' :
                        ...
                        {todayReport.status === 'Approved' && (
                          <span className="bg-emerald-100 text-emerald-800 text-[10px] font-black px-2.5 py-1 rounded-full uppercase tracking-wider border border-emerald-200">
                            Score: {totalReportScore} / 100
                          </span>
                        )}
                      </div>
                      
                      <p className="text-xs font-semibold text-slate-600 leading-relaxed">
                        {todayReport.status === 'Approved' ? "Excellent work! Your mentor has reviewed and approved today's mission accountability report." :
                         todayReport.status === 'Needs Improvement' || todayReport.status === 'Resubmit Required' ? "Your mentor reviewed your work and noted areas that need adjustment. Please view section feedback below." :
                         todayReport.status === 'Rejected' ? "This submission was rejected by your mentor. Please check the feedback remarks and correct your submission." :
                         "Your submission is queued in the system. Your mentor will review the attachments and assign scores/remarks."}
                      </p>

                      {todayReport.remarks && (
                        <div className="mt-3 p-4 bg-white/80 border border-slate-100 rounded-2xl shadow-xs text-left">
                          <p className="text-[10px] font-black text-slate-400 uppercase mb-1 flex items-center gap-1">
                            <MessageSquare size={10} /> Overall Mentor Remarks:
                          </p>
                          <p className="text-xs text-slate-700 font-extrabold italic leading-normal">
                            "{todayReport.remarks}"
                          </p>
                        </div>
                      )}
              ...
              <div className="space-y-4">
                ...
                {isSubmissionLockedByTime() ? (
                  <div className="bg-gradient-to-r from-indigo-50/50 via-slate-50 to-indigo-50/50 border-2 border-indigo-100 rounded-3xl p-8 text-center shadow-sm space-y-4">
                    <div className="w-14 h-14 bg-indigo-50 text-indigo-600 rounded-full flex items-center justify-center mx-auto shadow-sm border border-indigo-100">
                      <Clock size={28} className="animate-pulse" />
                    </div>
                    <div className="space-y-2">
                      <h3 className="text-sm font-black text-slate-900 uppercase tracking-wider">Submission Queue Locked</h3>
                      <p className="text-xs text-slate-500 font-medium leading-relaxed max-w-sm mx-auto">
                        Your previous submission has been approved! The next day's submission window will open at <strong className="text-indigo-600 font-extrabold">9:00 AM</strong>. Please utilize this time to prepare your studies.
                      </p>
                    </div>
                    <div className="inline-block bg-indigo-50 px-4 py-1.5 rounded-full text-[10px] font-black text-indigo-700 uppercase tracking-widest border border-indigo-100/50">
                      Opens Daily at 09:00 AM
                    </div>
                  </div>
                ) : (
                  SECTIONS_DEF.map((sec) => {
                    ...
                    const canSubmit = !isApproved; // Allow resubmission or appending files while pending review
                                ...
                                <div className="space-y-4">
                                  ...
                                  <div className="space-y-2">
                                    ...
                                    <div className="grid grid-cols-4 gap-2">
                                      {/* Add button trigger */}
                                      {isSectionSubmissionBlocked(sec.id) ? (
                                        <div className="aspect-square bg-slate-50 border-2 border-dashed border-slate-200 rounded-xl flex flex-col items-center justify-center text-slate-300 cursor-not-allowed">
                                          <Lock size={16} className="mb-0.5 text-slate-400" />
                                          <span className="text-[7px] font-black uppercase text-slate-400">Locked</span>
                                        </div>
                                      ) : (
                                  ...
                                  {isSectionSubmissionBlocked(sec.id) ? (
                                    <div className="w-full py-4 bg-slate-100 border border-slate-200 text-slate-400 rounded-2xl font-black text-[10px] uppercase tracking-widest text-center flex items-center justify-center gap-1.5 cursor-not-allowed">
                                      <Lock size={12} />
                                      Locked (Wait {getSectionTimeRemainingText(sec.id)} to submit)
                                    </div>
                                  ) : (
                                    <button 
                ...
                <div className="flex items-start gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-slate-400 mt-1 leading-relaxed">
                      Approved leave allows you to excuse yourself from active daily mission schedules. Leave will count toward your overall preparation and target days keeping your timeline optimized.
                    </p>
                    <button 
