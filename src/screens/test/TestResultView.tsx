...
import { 
  ...
  Share2, Download, Copy, CheckCircle, BookOpen, Brain, ListCollapse, ThumbsUp, ChevronDown, ChevronUp
} from 'lucide-react';
...
export default function TestResultView({ testId, attemptId, onBack, onPracticeIncorrect }: Props) {
  ...
  const isLocked = !isMentor && !userProfile;
        ...
        <div className="flex items-center space-x-3 p-4">
           ...
           {!isMentor && (
              <button 
                ...
                className="p-2.5 bg-indigo-50 text-indigo-600 rounded-xl hover:bg-indigo-100 transition-all flex items-center gap-2 border border-indigo-100"
              >
         ...
         {activeTab === 'solutions' && (
           <div className="max-w-3xl mx-auto space-y-4 pb-20">
             ...
             {[test.questions[currentQuestionIdx]].map((q, i) => {
               ...
               return (
                 <div key={`${q.id || 'q'}-${idx}`} className="bg-white p-5 rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
                   ...
                   <div className="text-slate-800 text-sm mb-4 leading-relaxed"><MathRenderer content={q.text} formula_latex={q.formula_latex} /></div>\n\n                    <MathDiagram metadata={q.diagramMetadata} diagram_svg={q.diagram_svg} />
                        ...
                        <div className="space-y-6">
                            ...
                            {q.ruleOrTheorem && (
                              <div className="bg-purple-50 p-4 rounded-xl border border-purple-100 mb-4">
                                ...
                                <div className="text-purple-900 text-xs leading-relaxed"><MathRenderer content={q.ruleOrTheorem} /></div>
                              </div>
                            )}
                            ...
                            {q.examApproach && (
                              <div className="bg-amber-50 p-4 rounded-xl border border-amber-100 mb-4">
                                ...
                                <div className="text-amber-900 text-xs leading-relaxed"><MathRenderer content={q.examApproach} /></div>
                              </div>
                            )}
                            ...
                            {q.stepwiseSolution && q.stepwiseSolution.length > 0 && (
                              <div>
                                ...
                                <div className="space-y-2">
                                  {q.stepwiseSolution.map((step, idx) => (
                           ...
                           {q.solution?.youtubeUrl && (
                             <div className="rounded-2xl overflow-hidden bg-slate-900 aspect-video relative group">
                                <iframe 
                                  src={`https://www.youtube.com/embed/${q.solution.youtubeUrl.includes('v=') ? q.solution.youtubeUrl.split('v=')[1].split('&')[0] : q.solution.youtubeUrl.split('/').pop()}`}
                           ...
                           {q.stepwiseSolution && q.stepwiseSolution.length > 0 ? (
                                 ...
                                 {q.stepwiseSolution.map((step, sIdx) => (
                                   <div key={sIdx} className="flex gap-4">
                                     <div className="flex flex-col items-center">
                                       ...
                                       {sIdx < (q.stepwiseSolution?.length || 0) - 1 && (
                             ...
                             <div>
                               ...
                               <div className="text-slate-700 text-sm leading-relaxed"><MathRenderer content={q.explanation} /></div>
                             </div>
                           )}
            ...
            >
              ...
              <div className="p-6 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h3 className="font-black text-slate-900 text-sm uppercase tracking-tight">Share Achievement</h3>
                    <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest">Victory Card</p>
                  </div>
                </div>
                <button 
              ...
              {/* The Result Card to Capture */}
              <div className="p-8">
                <div 
                  ref={cardRef}
                  className="bg-slate-50 rounded-[2rem] p-8 border border-slate-200 relative overflow-hidden shadow-inner"
                >
                  {/* Decorative Elements */}
                  ...
                  <div className="relative z-10 text-center space-y-6">
                    <div className="inline-flex flex-col items-center">
                       <div className="w-16 h-16 bg-white rounded-3xl shadow-lg flex items-center justify-center mb-3 border border-indigo-50">
                          <Award className="w-8 h-8 text-indigo-600" />
                       </div>
                       <div className="px-4 py-1 bg-indigo-600 text-white rounded-full text-[10px] font-black uppercase tracking-widest">MISSION SELECTION</div>
                    </div>

                    <div>
                       <h4 className="text-lg font-black text-slate-900 leading-tight mb-1">{test.title}</h4>
                       <p className="text-xs text-slate-500 font-bold">Successfully Completed</p>
                    </div>

                    <div className="bg-white rounded-2xl p-6 shadow-sm border border-slate-100 grid grid-cols-2 gap-4">
                       <div className="text-center">
                          <div className="text-[10px] text-slate-400 font-black uppercase mb-1">Score</div>
                          <div className="text-2xl font-black text-indigo-600 tabular-nums">{attempt.marks}</div>
                          <div className="text-[9px] text-slate-400 font-bold">out of {test.maximumMarks}</div>
                       </div>
                       <div className="text-center">
                          <div className="text-[10px] text-slate-400 font-black uppercase mb-1">Accuracy</div>
                          <div className="text-2xl font-black text-emerald-600 tabular-nums">{(attempt.percentage ?? 0).toFixed(0)}%</div>
                          <div className="text-[9px] text-slate-400 font-bold">Performance</div>
                       </div>
                    </div>
                    ...
                    <div className="pt-4 border-t border-slate-200/50">
                       <p className="text-[9px] text-slate-400 font-bold italic tracking-wide">Generated via Mission Selection Official App</p>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="p-6 pt-0 flex flex-col gap-3">
                <div className="grid grid-cols-2 gap-3">
                  <button 
                    ...
                    className="flex items-center justify-center gap-2 py-3.5 bg-slate-900 text-white rounded-2xl font-black text-xs uppercase tracking-widest hover:bg-slate-800 disabled:opacity-50 transition-all active:scale-[0.98] shadow-lg shadow-slate-200"
                  >
                  ...
                  <button 
                    ...
                    className="flex items-center justify-center gap-2 py-3.5 bg-white border-2 border-slate-200 text-slate-700 rounded-2xl font-black text-xs uppercase tracking-widest hover:bg-slate-50 transition-all active:scale-[0.98]"
                  >
                ...
                <p className="text-center text-[10px] text-slate-400 font-bold uppercase tracking-wider mt-2">
                  Proudly share your performance with your batch mates!
                </p>
              </div>
            </motion.div>
          </div>
