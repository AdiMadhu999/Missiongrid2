...
export default function MentorTestList() {
  ...
  const handleShareToStudents = async (test: Test) => {
    let link = '';
    if (test.shareableId) {
  ...
  const handleShareToCommunity = async (test: Test) => {
    if (userProfile?.role !== "mentor" && userProfile?.role !== "primary-mentor" && userProfile?.role !== "admin") {
      toast.error("Only mentors can share tests to the community!");
      return;
    }
    try {
      setLoading(true);
      ...
      const q = query(collection(db, 'dailyTests'), where('testId', '==', test.id), limit(1));
      const snap = await getDocs(q);
      if (!snap.empty) {
        toast.error("This test is already shared to the community!");
        return;
      }
      ...
      // Update test document to indicate it's shared
      await TestService.updateTest(test.id, { shareToCommunity: true });
          ...
          >
            <div className="flex items-start justify-between gap-3 mb-2">
              ...
              <div className="shrink-0 flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                {(userProfile?.role === "mentor" || userProfile?.role === "primary-mentor" || userProfile?.role === "admin") && test.isPublic && test.shareableId && (
                  <button
                    onClick={() => {
                      navigator.clipboard.writeText(`${window.location.origin}/live/${test.shareableId}`);
            ...
            <div className="flex flex-wrap items-center gap-1.5 pt-1.5 pb-2 border-t border-slate-100/80">
              <button
                onClick={() => handleShareToStudents(test)}
                className="flex flex-1 justify-center items-center gap-1.5 text-indigo-600 bg-indigo-50 hover:bg-indigo-100 py-2 rounded-xl text-[10px] font-black uppercase tracking-wider transition-colors"
              >
                <Share2 className="w-3 h-3" /> <span>Students</span>
              </button>
              {(userProfile?.role === "mentor" || userProfile?.role === "primary-mentor" || userProfile?.role === "admin") && (
                <button
                  onClick={() => handleShareToCommunity(test)}
                  className="flex flex-1 justify-center items-center gap-1.5 text-pink-600 bg-pink-50 hover:bg-pink-100 py-2 rounded-xl text-[10px] font-black uppercase tracking-wider transition-colors"
                >
                  <Globe className="w-3 h-3" /> <span>Community</span>
                </button>
              )}
            </div>
          </div>
