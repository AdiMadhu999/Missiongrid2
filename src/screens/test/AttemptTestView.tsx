...
import { Clock, Play, FileText, Award, ChevronLeft, ChevronRight, CheckCircle2, AlertTriangle, AlertCircle, Menu, X, Eye, Bookmark, WifiOff, Maximize, Languages } from 'lucide-react';
    ...
    <div className="bg-white rounded-3xl p-6 shadow-sm border border-slate-100 mb-6">
      ...
      <div className="prose prose-slate max-w-none mb-8">
        <div className="text-lg font-bold text-slate-900 leading-relaxed">
           <MathRenderer content={language === 'bn' && question.text_bn ? question.text_bn : question.text} formula_latex={question.formula_latex} />
        </div>
        {question.diagramMetadata && (
          <div className="mt-6 flex justify-center bg-slate-50 rounded-2xl p-4">
            <MathDiagram metadata={question.diagramMetadata} diagram_svg={question.diagram_svg} />
          </div>
        )}
      </div>

      <div className="space-y-3">
        {(language === 'bn' && question.options_bn?.length ? question.options_bn : question.options || []).map((opt, i) => {
          const originalOpt = question.options?.[i] || opt;
          const isSelected = Array.isArray(answer?.value) 
            ? answer.value.includes(originalOpt) 
            : answer?.value === originalOpt;
            ...
            <button
              ...
              onClick={() => {
                if (question.type === 'MCQ' || question.type === 'Boolean' || question.type === 'Integer') {
                  onAnswerChange(question.id, originalOpt);
                } else {
                  const current = Array.isArray(answer?.value) ? answer.value : [];
                  const next = current.includes(originalOpt) 
                    ? current.filter((v: string) => v !== originalOpt) 
                    : [...current, originalOpt];
...
export default function AttemptTestView({ testId, onExit, forceNewAttempt }: Props) {
  ...
  // CBT Question status statistics
  const stats = useMemo(() => {
    if (!test || !test.questions) return { answered: 0, notAnswered: 0, marked: 0, ansAndMarked: 0, notVisited: 0 };
    let answered = 0;
    let notAnswered = 0;
    let marked = 0;
    let ansAndMarked = 0;
    ...
    test.questions.forEach((qu: any) => {
      const ansValue = attempt?.answers[qu.id]?.value;
      const isAns = ansValue !== undefined && (Array.isArray(ansValue) ? ansValue.length > 0 : ansValue !== '');
      const isMark = markedForReview.includes(qu.id);
      const isVisit = visitedQuestions.includes(qu.id);
    ...
    return { answered, notAnswered, marked, ansAndMarked, notVisited };
  }, [test?.questions, attempt?.answers, markedForReview, visitedQuestions]);
  ...
  const loadTestAndAttempt = async () => {
    ...
    try {
      ...
      if (!isMentor && !t.isPractice && !t.shareToCommunity && !t.isPublic) {
        if (isGuest) {
          toast.error("Security Alert: This test is not public. Please log in.");
          onExit();
          return;
        } else if (isStudent) {
      ...
      <div className="flex items-center gap-3">
        <div className="flex bg-slate-100 p-1 rounded-lg">
          <button
            ...
            className={`px-3 py-1 text-xs font-bold rounded-md transition-colors ${language === 'en' ? 'bg-white text-indigo-700 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}
          >
            EN
          </button>
          <button
            onClick={() => onLanguageChange('bn')}
            className={`px-3 py-1 text-xs font-bold rounded-md transition-colors ${language === 'bn' ? 'bg-white text-indigo-700 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}
          >
            BN
          </button>
        </div>
          ...
          <div className="max-w-xl w-full bg-white rounded-3xl shadow-xl border border-slate-100 p-6 md:p-8 space-y-6">
            ...
            <div className="p-4 bg-indigo-50/50 rounded-2xl border border-indigo-100/80 space-y-3">
              <div className="flex items-center gap-2">
                ...
                <h4 className="font-black text-xs text-indigo-900 uppercase tracking-wider">
                  Choose Default Exam Language / পরীক্ষার ভাষা
                </h4>
              </div>
              <div className="grid grid-cols-2 gap-3">
                <button
                  ...
                  className={`py-3 px-4 rounded-xl font-extrabold text-sm transition-all border-2 flex items-center justify-center gap-2 ${
                    language === 'en'
              ...
              <p className="text-[10px] text-slate-500 font-bold text-center">
                {language === 'en' 
                  ? "Note: You can still switch languages inside the exam if needed."
                  : "দ্রষ্টব্য: পরীক্ষার সময়ও আপনি প্রয়োজনমতো ভাষা পরিবর্তন করতে পারবেন।"}
              </p>
            </div>
  ...
  return (
    <div className="fixed inset-0 z-[60] bg-slate-50 flex flex-col pt-[env(safe-area-inset-top)]">
      ...
      <div className="flex-1 flex overflow-hidden relative">
         ...
         <div className="flex-1 overflow-y-auto p-4 md:p-6 pb-24">
            <div className="max-w-2xl mx-auto bg-white rounded-2xl shadow-sm border border-slate-100 p-6">
                 ...
                 <div className="flex items-center space-x-2">
                   ...
                   {test.negativeMarking && q.negativePoints && (
                     <span className="text-xs font-semibold text-red-600 bg-red-50 px-2 py-0.5 rounded">-{q.negativePoints}</span>
                           ...
                           <div className="text-slate-900 text-lg leading-relaxed mb-6"><MathRenderer content={language === 'bn' && q.text_bn ? q.text_bn : q.text} formula_latex={q.formula_latex} /></div>
              ...
              />
               ...
               {/* Inputs */}
               <div className="space-y-3">
                 {q.type === 'MCQ' && (language === 'bn' && q.options_bn?.length ? q.options_bn : q.options || []).map((opt, i) => (
                 ...
                 {q.type === 'Boolean' && ['True', 'False'].map((opt) => (
                    <label key={opt} className={`flex items-start p-4 rounded-xl border-2 cursor-pointer transition-colors ${ans === opt ? 'border-primary-500 bg-primary-50' : 'border-slate-200 hover:bg-slate-50'}`}>
                      ...
                      <span className="ml-3 text-slate-700 font-medium"><MathRenderer content={language === 'bn' ? (opt === 'True' ? 'সত্য' : 'মিথ্যা') : opt} /></span>
                    </label>
                  ))}
                 ...
                 {q.type === 'MSQ' && (language === 'bn' && q.options_bn?.length ? q.options_bn : q.options || []).map((opt, i) => (
         ...
         {/* Right Column: Persistent Desktop CBT Question Palette & Candidate Panel */}
         <div className="hidden lg:flex w-80 shrink-0 flex-col border-l border-slate-200 bg-slate-50 overflow-hidden h-full">
           ...
           <div className="p-4 border-b border-slate-200 bg-white flex items-center gap-3">
             <div className="w-14 h-14 rounded-lg bg-slate-100 border border-slate-200 flex items-center justify-center overflow-hidden shrink-0">
               {userProfile?.photoUrl || currentUser?.photoURL ? (
                 <img src={userProfile?.photoUrl || currentUser?.photoURL || ''} alt="Candidate" className="w-full h-full object-cover" />
               ) : (
                 <div className="w-full h-full bg-indigo-50 flex items-center justify-center text-indigo-500 font-extrabold text-base">
                   {(userProfile?.name || currentUser?.displayName || 'Candidate').slice(0, 2).toUpperCase()}
                 </div>
               )}
             </div>
             <div className="min-w-0 flex-1">
               <div className="text-[10px] uppercase font-black text-slate-400 tracking-wider">Candidate Name</div>
               <div className="font-extrabold text-slate-800 text-sm truncate">
                 {userProfile?.name || currentUser?.displayName || 'Guest Candidate'}
               </div>
               <div className="text-[10px] font-mono text-slate-500 mt-0.5">
                 Roll No: <span className="font-bold text-slate-700">{currentUser?.uid ? parseInt(currentUser.uid.replace(/\D/g, '').slice(0, 8)) || '10842951' : '10842951'}</span>
               </div>
             </div>
           </div>

           <div className="px-4 py-2 bg-slate-100 border-b border-slate-200 text-[10px] font-bold uppercase tracking-wider text-slate-600 flex justify-between items-center shrink-0">
             <span className="truncate">Subject: {test.title}</span>
           </div>

           {/* Desktop Question Palette List */}
           <div className="p-4 flex-1 overflow-y-auto">
             <h4 className="font-black text-xs uppercase tracking-wider text-slate-500 mb-3 flex items-center justify-between">
               <span>Question Palette</span>
               <span className="text-[10px] font-mono px-2 py-0.5 bg-slate-200 text-slate-600 rounded-md font-bold">
                 {stats.answered}/{test.questions.length}
               </span>
             </h4>
             ...
             <div className="grid grid-cols-5 gap-2 pb-6">
               {test.questions.map((qu, idx) => {
                 ...
                 // Standard CBT shape and color styles
                 let shapeStyle = '';

                 if (isAnswered && isMarked) {
                   shapeStyle = 'bg-violet-600 border-violet-600 text-white rounded-full';
                 } else if (isMarked) {
                   shapeStyle = 'bg-violet-500 border-violet-500 text-white rounded-full';
                 } else if (isAnswered) {
                   shapeStyle = 'bg-emerald-600 border-emerald-600 text-white rounded-t-xl rounded-b-sm';
                 } else if (isVisited) {
                   shapeStyle = 'bg-rose-500 border-rose-500 text-white rounded-b-xl rounded-t-sm';
                 } else {
                   shapeStyle = 'bg-slate-100 border-slate-300 text-slate-600 rounded-md';
                 }

                 const isLocked = test.isFullMockTest && test.timingMode === 'section' && qu.sectionId !== activeSection?.id;

                 return (
                   <button
                     key={`desktop-${qu.id || 'qu'}-${idx}`}
                     disabled={isLocked}
                     onClick={() => {
                       flushQuestionTime();
                       setCurrentIdx(idx);
                     }}
                     className={`aspect-square text-xs font-black flex items-center justify-center border transition-all relative ${shapeStyle} ${
                       isLocked ? 'opacity-40 cursor-not-allowed' : currentIdx === idx ? 'ring-4 ring-indigo-200 border-indigo-600 scale-105 z-10' : ''
                     }`}
                   >
                     {isLocked ? '🔒' : idx + 1}
                     {isAnswered && isMarked && (
           ...
           {/* CBT Legend Summary Stats & Submit Test Button */}
           <div className="p-4 border-t border-slate-200 bg-white space-y-3 shrink-0">
             <div className="grid grid-cols-2 gap-2 text-[10px] font-bold uppercase tracking-wider">
               <div className="flex items-center gap-1.5">
                 <div className="w-4 h-4 bg-emerald-600 rounded-t-md rounded-b-sm flex items-center justify-center text-[8px] font-black text-white shrink-0">
                   {stats.answered}
                 </div>
                 <span className="text-slate-500 truncate font-medium">Answered</span>
               </div>
               <div className="flex items-center gap-1.5">
                 <div className="w-4 h-4 bg-rose-500 rounded-b-md rounded-t-sm flex items-center justify-center text-[8px] font-black text-white shrink-0">
                   {stats.notAnswered}
                 </div>
                 <span className="text-slate-500 truncate font-medium">Not Ans</span>
               </div>
               <div className="flex items-center gap-1.5">
                 <div className="w-4 h-4 bg-violet-500 rounded-full flex items-center justify-center text-[8px] font-black text-white shrink-0">
                   {stats.marked}
                 </div>
                 <span className="text-slate-500 truncate font-medium">Marked</span>
               </div>
               <div className="flex items-center gap-1.5">
                 <div className="w-4 h-4 bg-violet-600 rounded-full relative flex items-center justify-center text-[8px] font-black text-white shrink-0">
                   <div className="absolute -top-0.5 -right-0.5 w-1.5 h-1.5 bg-emerald-500 rounded-full border border-white" />
                   {stats.ansAndMarked}
                 </div>
                 <span className="text-slate-500 truncate font-medium">Ans & Marked</span>
               </div>
               <div className="flex items-center gap-1.5 col-span-2">
                 <div className="w-4 h-4 bg-slate-100 border border-slate-300 rounded-md flex items-center justify-center text-[8px] font-black text-slate-600 shrink-0">
                   {stats.notVisited}
                 </div>
             ...
             <button
               ...
               className="w-full py-3 bg-red-600 hover:bg-red-700 text-white rounded-xl font-black text-xs uppercase tracking-widest shadow-lg shadow-red-100 transition-all active:scale-95"
             >
               Submit Test
             </button>
           </div>
      ...
      {/* Footer Navigation */}
      <div className="bg-white border-t border-slate-200 p-4 pb-[calc(env(safe-area-inset-bottom,12px)+12px)] flex flex-col gap-3 z-20 lg:mr-80 lg:border-r">
         {/* Action Tier */}
