...
export default function StudentTargetView({ 
  ...
  const getCalendarDays = (centerDateStr: string) => {
    ...
    let centerDate: Date;
    try {
      centerDate = new Date(centerDateStr + "T12:00:00");
  ...
  const renderTelegramTargetCard = (target: DailyTarget, idx: number) => {
    ...
    const prevMissionApproved = isFirstDay || studentReports.some(r => r.date === prevDateStr && r.status === 'Approved');
    const currentMissionSubmitted = studentReports.some(r => r.date === targetDateStr && ['Approved', 'Pending', 'Under Review', 'Submitted', 'Late Submission'].includes(r.status));

    const canMarkDone = currentMissionSubmitted;
              ...
              <div className="py-6 flex flex-col items-center justify-center text-center space-y-3">
                ...
                <div>
                  ...
                  <p className="text-slate-500 text-[10px] font-semibold max-w-xs mx-auto leading-relaxed mt-1">
                    This target will unlock automatically as soon as your daily mission proof for {getRelativeDateLabel(prevDateStr)} is approved by your mentor.
                  </p>
                </div>
                <button 
                  onClick={(e) => { 
          ...
          <div className="space-y-4">
                ...
                {target.youtubeLinks?.map((l, i) => {
                  ...
                  return (
                    <a 
                      ...
                      target="_blank" 
                      rel="noreferrer" 
                      className={`flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg text-[10px] font-bold ${isDarkTheme ? 'bg-white/10 hover:bg-white/20' : 'bg-red-50 text-red-700 hover:bg-red-100'}`}
                    >
                      <Youtube size={12} /> Class {i+1} (Redirect)
                    </a>
                  );
                })}
            ...
            <div className="flex items-center justify-between pt-3 border-t border-black/5">
              ...
              <div className="flex flex-col items-end gap-1">
                ...
                {!canMarkDone && (() => {
                  const currentReport = studentReports.find(r => r.date === targetDateStr);
                  return (
                    <p className={`text-[8px] font-bold mt-0.5 ${isDarkTheme ? 'text-rose-300' : 'text-rose-600'}`}>
                      {currentReport?.status === 'Pending' 
                        ? '⏳ Mission proof pending mentor review.' 
                        : currentReport?.status === 'Needs Improvement' || currentReport?.status === 'Resubmit Required'
                          ? '❌ Mission proof needs correction. Please resubmit!'
                          : currentReport?.status === 'Rejected'
                            ? '❌ Mission proof rejected. Please resubmit!'
                            : "🔒 Required: approved daily mission proof."}
                    </p>
                  );
                })()}
              </div>
  ...
  }, [targets, searchQuery, searchDate]);
  ...
  const selectedTargets = React.useMemo(() => {
    return targets.filter(t => {
      const tDate = t.scheduledFor || safeSplitDate(t.createdAt);
      return tDate === selectedDateStr && t.status === 'published';
    });
  }, [targets, selectedDateStr]);

  const todayTargets = React.useMemo(() => {
    return targets.filter(t => t.status === 'published');
  }, [targets]);
  ...
  return (
    ...
    >
        ...
        <div className="max-w-2xl mx-auto flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
          ...
          <div className="flex items-center gap-3 sm:text-right sm:flex-row-reverse">
            ...
            <div>
              ...
              <h3 className="text-sm font-black text-indigo-700">{todayTargets.length} Active Targets</h3>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="flex-1 p-4 space-y-6 flex flex-col">
        {/* 1. TODAY'S TARGETS TAB (Feed Style with Calendar Date logic) */}
        {activeTab === 'today' && (
          <div className="flex flex-col gap-4 max-w-2xl mx-auto w-full py-4">
            ...
            <div className="bg-white rounded-3xl p-4 border border-slate-200/60 shadow-sm space-y-3">
              <div className="flex items-center justify-between">
                <span className="text-[11px] font-black text-slate-700 uppercase tracking-widest flex items-center gap-1">
                ...
                <input 
                  ...
                  className="bg-emerald-50 border border-emerald-150 rounded-xl px-2.5 py-1 text-xs font-bold text-emerald-800 focus:outline-none"
                />
              </div>

              {/* Horizontal Scrollable Row of Days */}
              <div className="flex gap-2 overflow-x-auto no-scrollbar pb-1 pt-1">
                {getCalendarDays(selectedDateStr).map((day) => {
                  ...
                  return (
                    <button
                      ...
                      className={`flex flex-col items-center justify-center p-2 rounded-2xl min-w-[56px] text-center transition-all ${
                        isSelected 
                    ...
                    >
                      <span className={`text-[8px] uppercase font-black tracking-wider ${isSelected ? 'text-emerald-100' : 'text-slate-400'}`}>
                        {day.dayName}
                      </span>
                      <span className="text-sm font-black mt-0.5">
                        {day.dayNum}
                      </span>
                      <span className={`text-[7px] uppercase font-bold tracking-widest mt-0.5 ${
                        isSelected ? 'text-emerald-200' : day.isFuture ? 'text-amber-500' : 'text-slate-450'
                      }`}>
                        {day.isFuture ? '🔒 Locked' : day.isToday ? 'Today' : day.monthName}
                      </span>
                    </button>
                  );
                })}
            ...
            {/* Render selected date's targets */}
            {selectedDateStr > new Date().toISOString().split('T')[0] ? (
              /* Future locked state */
              <div className="text-center py-20 bg-white/85 border border-slate-200 rounded-[2rem] p-6 shadow-sm">
                <div className="w-16 h-16 bg-amber-50 rounded-full flex items-center justify-center mx-auto mb-4 text-amber-500">
                  <Lock size={28} />
                </div>
                <h3 className="text-slate-900 font-extrabold mb-1 text-sm sm:text-base">Target Date Locked</h3>
                <p className="text-slate-500 text-[11px] sm:text-xs font-semibold max-w-xs mx-auto leading-relaxed">
                  Targets for future calendar dates are currently locked. They will unlock automatically when that day begins!
                </p>
              </div>
            ) : selectedTargets.length === 0 ? (
              <div className="text-center py-20 bg-white/80 border border-slate-200/55 rounded-[2rem] p-6 shadow-sm">
                <div className="w-16 h-16 bg-emerald-50 rounded-full flex items-center justify-center mx-auto mb-4 text-emerald-600">
                  <Calendar size={28} />
                </div>
                <h3 className="text-slate-900 font-extrabold mb-1 text-sm sm:text-base">No Target for this Date</h3>
                <p className="text-slate-500 text-[11px] sm:text-xs font-semibold max-w-xs mx-auto leading-relaxed">
                  There are no targets published for {getRelativeDateLabel(selectedDateStr)}. Please check back later or choose another date!
                </p>
              </div>
            ) : (
              selectedTargets.map((target, idx) => renderTelegramTargetCard(target, idx))
            )}
          </div>
        )}
