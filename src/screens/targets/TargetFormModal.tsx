...
export default function TargetFormModal({ target, onClose, onSaved }: Props) {
  ...
  const [targetDate, setTargetDate] = useState<string>(() => {
    if (target) {
      if (target.scheduledFor) return target.scheduledFor;
      const fallback = (target as any).createdAt ? (target as any).createdAt.split('T')[0] : '';
  ...
  const [embedInput, setEmbedInput] = useState(true);
  ...
  const handleAddLink = (type: 'pdf' | 'youtube' | 'website') => {
    ...
    if (type === 'youtube' && youtubeInput.trim()) {
      setYoutubeLinks([...youtubeLinks, `${youtubeInput.trim()}|false`]);
      setYoutubeInput('');
    }
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const targetTimePart = target?.createdAt ? target.createdAt.split('T')[1] || '12:00:00.000Z' : new Date().toISOString().split('T')[1];
      const newCreatedAt = `${selectedDate}T${targetTimePart}`;
        ...
        <div className="flex-1 overflow-y-auto p-3.5 space-y-4 pb-24 bg-slate-50/50">
          ...
          <div className="bg-white p-3.5 md:p-4 rounded-2xl border border-slate-100 shadow-sm space-y-3">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-4 gap-2.5">
              ...
              <div className="sm:col-span-2">
                <label className="block text-[9px] font-black text-slate-400 uppercase tracking-wider mb-1">Target Calendar Date</label>
                <input 
                  type="date" 
                  value={targetDate} 
                  onChange={(e) => setTargetDate(e.target.value)} 
                  className="w-full border border-slate-205 rounded-xl px-2.5 py-1.5 text-xs font-bold focus:ring-1 focus:ring-indigo-500 focus:outline-none text-slate-800 bg-indigo-50/50" 
                />
              </div>
            </div>
                    ...
                    <div className="flex flex-wrap gap-1.5 mt-2">
                      {youtubeLinks.map((y, i) => (
                        <span key={`youtube-link-${i}-${y.substring(0, 10)}`} className="text-[10px] font-bold text-slate-700 bg-slate-100 px-2.5 py-1 rounded-xl flex items-center gap-1.5 border border-slate-205">
                          <span className="truncate max-w-[120px]">{y.split('|')[0]}</span>
                          <span className="text-[8px] font-black text-amber-600">[Redirect Link]</span>
