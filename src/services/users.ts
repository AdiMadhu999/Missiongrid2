...
export const getUsers = async (): Promise<User[]> => {
  ...
  return dedupeRequest('getUsers', async () => {
    ...
    if (isMentorUser) {
      try {
        const privSnap = await getDocs(query(collection(db, 'users_private')));
        privSnap.forEach(d => {
...
export const createUserProfile = async (userData: Partial<User>) => {
  ...
  const currentDeviceType = Capacitor.getPlatform() === 'android' ? 'android' : 'desktop';
  const androidDeviceId = currentDeviceType === 'android' ? deviceId : '';
  const desktopDeviceId = currentDeviceType === 'desktop' ? deviceId : '';
  ...
  const publicData = {
    ...
    isPremium: true, // Default to 20 days active premium access for new signups
    ...
    premiumExpiryDate: expiryDate.toISOString(), // Required: 20 days from registration
    premiumDays: 20, // Required field
    premiumType: 'FREE_TRIAL',
    trialDays: 20,
    remainingPremiumDays: 20,
...
export const updateUserProfile = async (idOrMobile: string, data: Partial<User>) => {
  ...
  if (!isMentor) {
    const restrictedFields = [
      ...
      'deviceId', 'androidDeviceId', 'desktopDeviceId', 'deviceName', 'deviceModel', 'operatingSystem', 'browser', 'browserInfo', 'deviceInfo', 'currentDevice',
  ...
  const publicFields = [
    ...
    'deviceId', 'androidDeviceId', 'desktopDeviceId', 'deviceName', 'deviceModel', 'operatingSystem', 'browser', 'browserInfo', 'deviceInfo', 'registrationIP', 'currentIP', 'lastLoginIP', 'registrationSource', 'currentBatch', 'lastSubmissionAt', 'lastSectionSubmissionAt'
  ];
  const privateFields = [
    'mobile', 'email', 'pin', 'uid', 'address', 'loginHistory',
    'registrationDateTime', 'registrationDate', 'registrationTime', 'registrationTimestamp',
    'browser', 'operatingSystem', 'deviceType', 'lastDeviceType', 'deviceId', 'androidDeviceId', 'desktopDeviceId', 'deviceName', 'deviceModel',
