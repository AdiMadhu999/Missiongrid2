...
export const MissionService = {
  submitDailyReport: async (report: Omit<DailyMissionReport, 'id' | 'status' | 'marks' | 'submittedAt'>): Promise<void> => {
    ...
    // Since the premium system is removed from the application, all students are granted full unlocked access.
    userData.isPremium = true;
    if (!userData.batchId) {
      throw new Error("Student is not assigned to any batch.");
    }
    if (userData.lastSubmissionAt) {
      const lastSub = new Date(userData.lastSubmissionAt).getTime();
      const now = Date.now();
      const twentyFourHours = 24 * 60 * 60 * 1000;
      if (now - lastSub < twentyFourHours) {
        const diffMs = twentyFourHours - (now - lastSub);
        const hoursLeft = Math.floor(diffMs / (1000 * 60 * 60));
        const minsLeft = Math.ceil((diffMs % (1000 * 60 * 60)) / (1000 * 60));
        throw new Error(`To prevent duplication, submission is blocked for 1 day. Please wait ${hoursLeft}h ${minsLeft}m before submitting again.`);
      }
    }
    ...
    try {
      ...
      if (snap.exists() && snap.data().status !== 'Rejected' && snap.data().status !== 'Needs Improvement' && snap.data().status !== 'Resubmit Required') {
        throw new Error(`Duplicate submission blocked. A mission report has already been submitted for ${report.date}.`);
      }
      ...
      const { publicRef } = await resolveUserDoc(report.userId);
  ...
  submitSectionReport: async (
    ...
    // Since the premium system is removed from the application, all students are granted full unlocked access.
    userData.isPremium = true;
    const lastSectionSubStr = userData.lastSectionSubmissionAt?.[sectionId];
    if (lastSectionSubStr) {
      const lastSub = new Date(lastSectionSubStr).getTime();
      const now = Date.now();
      const twentyFourHours = 24 * 60 * 60 * 1000;
      if (now - lastSub < twentyFourHours) {
        const diffMs = twentyFourHours - (now - lastSub);
        const hoursLeft = Math.floor(diffMs / (1000 * 60 * 60));
        const minsLeft = Math.ceil((diffMs % (1000 * 60 * 60)) / (1000 * 60));
        throw new Error(`To prevent duplication, submission for this section is blocked for 1 day. Please wait ${hoursLeft}h ${minsLeft}m before submitting again.`);
      }
    }
    ...
    try {
      ...
      // No duplicate submission check so students can submit as many files as they want
      const existingSectionAttachments = sections[sectionId]?.attachments || [];
      const mergedSectionAttachments = [...existingSectionAttachments, ...(sectionAttachments || [])];

      // Update specific section by appending notes and files
      sections[sectionId] = {
        ...sections[sectionId],
        submitted: true,
        submittedAt: new Date().toISOString(),
        status: 'Pending',
        marks: 0,
        notes: sectionNotes ? (sections[sectionId]?.notes ? `${sections[sectionId].notes}\n${sectionNotes}` : sectionNotes) : (sections[sectionId]?.notes || ''),
        attachments: mergedSectionAttachments
      };
      ...
      const { publicRef } = await resolveUserDoc(userId);
      const updatedSectionSubmissionMap = {
        ...(userData.lastSectionSubmissionAt || {}),
        [sectionId]: new Date().toISOString()
      };
      await updateDoc(publicRef, {
        lastSubmissionAt: new Date().toISOString(),
        lastSectionSubmissionAt: updatedSectionSubmissionMap,
        updatedAt: new Date().toISOString()
      });
    } catch (error) {
      handleFirestoreError(error, OperationType.WRITE, `${path}/${docId}`);
  ...
  reviewSectionsReport: async (
    ...
    try {
      ...
      const overallRemarks = Object.values(sectionReviews)[0]?.remarks || '';
      ...
      const updatedReport = sanitizeQuestionObject({
        ...
        remarks: overallRemarks || report.remarks || '',
        reviewedBy,
      ...
      // Reset consecutive missed missions and save last submission date on approved overall status
      if (overallStatus === 'Approved') {
        try {
          ...
          console.error("Failed to update student consecutive missed missions on reviewSectionsReport approval:", err);
        }
      }
      ...
      try {
        if (sanitizedReview.status === 'Approved') {
          try {
            ...
            console.error("Failed to update student consecutive missed missions on reviewReport approval:", err);
          }
