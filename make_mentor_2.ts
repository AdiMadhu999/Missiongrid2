...
async function run() {
    ...
    if (uid) {
        ...
        // Let's also check if there's any public profile or private profile having a legacy userId and fix it
        if (privateSnap.exists && privateSnap.data()?.userId !== mobile) {
            await privateRef.update({ userId: mobile });
        }
        if (userSnap.exists && userSnap.data()?.userId !== mobile) {
            await usersRef.update({ userId: mobile });
        }
