...
import {
  startBackgroundStudyJob,
  buildStudyMaterialPdfBuffer,
  getStudyPdfPath,
  STORAGE_PDF_DIR
} from "./server_study_jobs.ts";
import { 
  executeResilientAI, 
  poolStats, 
  getPoolKeys, 
...
async function processPremiumVerificationForUser(adminDb: any, doc: any, now: Date, yesterdayStr: string, authHeader: string | null = null) {
  ...
  // Return early bypass for all premium verifications
  if (true) {
    return { studentId, name: uData.name, status: 'PREMIUM', missedCount: 0, actionTaken: 'Bypassed' };
  }
  ...
  if (true) {
    ...
    var missedCount = uData.consecutiveMissedMissions || 0;
...
if (needsUpdate) {
    ...
    const studentUid = uData.uid || studentId;
    if (studentUid) {
      try {
        await syncCustomClaimsForUser(studentUid);
        console.log(`[Daily Job] Successfully synced custom claims for user ${studentUid}`);
      } catch (claimErr) {
        console.error(`[Daily Job] Failed to sync custom claims for user ${studentUid}:`, claimErr);
      }
    }
  }
...
if (getApps().length === 0) {
  ...
  if (process.env.FIREBASE_SERVICE_ACCOUNT_KEY) {
    ...
    try {
      ...
      console.error("[Server] Failed to parse FIREBASE_SERVICE_ACCOUNT_KEY, falling back to applicationDefault():", err);
      try {
        credential = applicationDefault();
      } catch (appDefaultErr) {
        console.error("[Server] Failed to load applicationDefault credential:", appDefaultErr);
      }
    }
  } else {
  ...
  const resolvedProjectId = process.env.GOOGLE_CLOUD_PROJECT || process.env.GCLOUD_PROJECT || projectId || "mission-selection-ultimate";
  const resolvedStorageBucket = process.env.FIREBASE_STORAGE_BUCKET || storageBucket || `${resolvedProjectId}.firebasestorage.app`;
  console.log(`[Server] Initializing Firebase Admin with project: ${resolvedProjectId}, bucket: ${resolvedStorageBucket}`);
  initializeApp({
    credential,
...
async function deleteDailySubmissionFiles() {
  console.log("[Scheduler] Starting automatic deletion of submission files older than 24 hours from Firebase Storage...");
  try {
    const bucketName = process.env.FIREBASE_STORAGE_BUCKET || storageBucket || `${projectId}.firebasestorage.app`;
    const bucket = getStorage().bucket(bucketName);
    const [files] = await bucket.getFiles({ prefix: "submissions/" });
    console.log(`[Scheduler] Found ${files.length} total submission files in storage.`);
    ...
    for (const file of files) {
      try {
        ...
        if (now - createdTime > oneDayMs) {
          await file.delete();
          deletedCount++;
          console.log(`[Scheduler] Deleted file older than 1 day: ${file.name} (Created: ${metadata.timeCreated})`);
        }
      } catch (err) {
        console.error(`[Scheduler] Failed to process or delete file ${file.name}:`, err);
      }
    }
    console.log(`[Scheduler] Successfully deleted ${deletedCount} submission files older than 1 day.`);
  } catch (error) {
    console.error("[Scheduler] Error deleting daily submission files:", error);
  }
}

// 1. Start listening to the port immediately
const server = app.listen(PORT, "0.0.0.0", () => {
  ...
  try {
    ...
    console.log("[Scheduler] Scheduled daily premium verification with timezone Asia/Kolkata");
  } catch (cronErr) {
    console.error("[Scheduler] Failed to schedule premium verification with timezone, falling back to system timezone:", cronErr);
    try {
      cron.schedule('0 9 * * *', () => {
        console.log("[Scheduler] Running daily premium verification...");
        performDailyPremiumVerification().catch(console.error);
      });
      console.log("[Scheduler] Scheduled daily premium verification with system timezone");
    } catch (fallbackErr) {
      console.error("[Scheduler] Critical: Failed to schedule premium verification:", fallbackErr);
    }
  }

  // Schedule hourly submission files deletion to automatically prune files older than 24 hours (1 day)
  try {
    cron.schedule('0 * * * *', () => {
      console.log("[Scheduler] Running hourly check for submission files deletion...");
      deleteDailySubmissionFiles().catch(console.error);
    }, {
      timezone: "Asia/Kolkata"
    });
    console.log("[Scheduler] Scheduled hourly submission files deletion with timezone Asia/Kolkata");
  } catch (cronErr) {
    console.error("[Scheduler] Failed to schedule hourly submission files deletion with timezone, falling back to system timezone:", cronErr);
    try {
      cron.schedule('0 * * * *', () => {
        console.log("[Scheduler] Running hourly check for submission files deletion...");
        deleteDailySubmissionFiles().catch(console.error);
      });
      console.log("[Scheduler] Scheduled hourly submission files deletion with system timezone");
    } catch (fallbackErr) {
      console.error("[Scheduler] Critical: Failed to schedule hourly submission files deletion:", fallbackErr);
    }
  }
});
...
app.post("/api/admin/send-push", requireMentor, async (req, res) => {
  const { receiverId, title, description, activityId, activityType } = req.body;
  ...
  try {
    const userDoc = await getFirestore(getApp(), "(default)").collection("users_private").doc(receiverId).get();
    const fcmToken = userDoc.data()?.fcmToken;

    if (!fcmToken) {
      return res.status(404).json({ error: "User has no FCM token registered" });
    }
...
// Endpoint for generating study material content
app.post("/api/mentor/generate-content", requireMentor, async (req, res) => {
  const { topic, tone, depth, language, isEnhanced, isNoteMaking, content: providedContent, includeMindmap, isFunnyMemoryMode = true } = req.body;
  ...
  try {
    const prompt = `
      You are an expert Academic Mentor, Author, and Curriculum Designer specializing in high-performance educational publication design.
      
      TASK: Create a professional, publication-ready study guide book chapter for the topic: "${topic}".
      LANGUAGE: ${language}.
      TONE: ${tone}.
      DEPTH: ${depth}.

      CRITICAL LANGUAGE POLICY (STRICT NON-NEGOTIABLE):
      - NEVER USE HINDI ANYWHERE IN THE GENERATED CONTENT.
      - Allowed languages: English, Bengali, or English + Bengali.
      - If bilingual mode is selected: Output English text first, followed by Bengali explanation. Never mix Hindi words or Hindi script.
      ...
      ${includeMindmap ? `
      MINDMAP GENERATION:
      - At the end of the content, provide a Mermaid Mindmap diagram.
      - Format:
        \`\`\`mermaid
        mindmap
          root((Topic Name))
            ...
            Node 2
              Subnode C
        \`\`\`
      - Ensure the mindmap summarizes the main structure of the study guide.
      ` : ''}

      MANDATORY 14-SECTION BOOK CHAPTER STRUCTURE:
      1. # [Clear, Professional Book Chapter Title]
      ...
      14. ## 😂 Funny Memory Mode (Visual & Phonetic Mnemonics)
         [IMPORTANT: Check if topic/material is English Vocabulary (Synonyms, Antonyms, OWS, Idioms, Phrasal Verbs, Spelling, Word Meaning, Confusing Words, Root Words).
          IF NOT VOCABULARY (e.g. Maths, Reasoning, GK, Tenses, Grammar rules): DO NOT generate this section at all!
          IF VOCABULARY: Generate a memory card for EVERY vocabulary word with these exact fields:
          - ### 📖 Word: [Word Name]
          - **📖 Meaning**: [Short exam-friendly meaning]
          - **😂 Funny Memory Trick**: [Funny, exaggerated story]
          - **🧠 Memory Connection**: [Why story connects to meaning]
          - **🔊 Sound Association**: [Phonetic Bengali/Indian sound link or visual story]
          - **🌟 Easy Example**: [Simple sentence]
          - **⚡ Quick Revision**: [One-line formula]
          - **🚫 Common Confusion**: [Similar confusing words]
         ]` : ''}
      ${includeMindmap ? '15. ## Concept Mindmap (Visual overview)' : ''}
      ...
      QUALITY RULES FOR FUNNY MEMORY MNEMONICS:
      - Every mnemonic must be original and prioritize memorability over comedy.
      - Never change the actual meaning of the word or generate misleading tricks.
      - Avoid offensive, political, religious, or inappropriate content.
      - Maintain a fun, enthusiastic, but strictly educational tone suitable for SSC, Banking, Railway, UPSC, State PSC exams.
    ...
    const content = await executeResilientAI(async (ai) => {
...
// Endpoint for generating PDF synchronously
app.post("/api/mentor/generate-pdf", requireMentor, async (req, res) => {
  const { content = "", topic = "MissionGrid Study Material", language = "english" } = req.body;
  
  try {
    const isBn = language.toLowerCase().includes('bengali') || language.toLowerCase().includes('bn');
    const edition = isBn ? 'bengali' : 'english';
    const pdfBuffer = await buildStudyMaterialPdfBuffer(content, topic, edition);
    const safeFilename = topic.replace(/[^a-zA-Z0-9_-]/g, '_').substring(0, 40) || 'Study_Guide';
    
    res.setHeader('Content-Type', 'application/pdf');
    res.setHeader('Content-Disposition', `attachment; filename="${safeFilename}_${isBn ? 'Bengali_Edition' : 'English_Edition'}_MissionGrid.pdf"`);
    res.send(pdfBuffer);
  } catch (err) {
...
// =========================================================================
// BACKGROUND STUDY MATERIAL GENERATION JOB ENDPOINTS
// =========================================================================

// Start asynchronous background study material generation job
app.post("/api/mentor/study-jobs/start", requireAuth, async (req, res) => {
  try {
    const { 
      topic, 
      tone = "Exam-Oriented", 
      ...
      isFunnyMemoryMode = false, 
      uploadQueue = [],
      content = ""
    } = req.body;

    if (!topic || typeof topic !== "string" || !topic.trim()) {
      return res.status(400).json({ error: "Please provide a valid study material topic." });
    }

    const limitCheck = checkAIUsageAllowed(0.5);
    if (!limitCheck.allowed) {
      return res.status(400).json({ 
        error: `Daily AI Spending limit reached (Limit: ₹${limitCheck.limit}, Spent Today: ₹${limitCheck.spentToday.toFixed(2)}). Please increase limit in Settings.` 
      });
    }

    const jobId = `study-${Date.now()}`;
    const user = (req as any).user;
    const createdBy = user?.uid || "mentor";

    const jobState: JobState = {
      id: jobId,
      jobType: "study_material",
      createdBy,
      preferences: "",
      topic: topic.trim(),
      tone,
      ...
      isFunnyMemoryMode,
      status: "queued",
      stage: "Queued in background generator",
      percent: 5,
      etaSeconds: 30,
      logs: [`[${new Date().toLocaleTimeString()}] Pipeline queued for "${topic.trim()}". Starting background AI generator...`],
      steps: ["Queued", "Processing Context", "AI Chapter Synthesis", "PDF Typesetting", "Completed"],
      currentStepIndex: 0,
    ...
    if (content) {
      jobState.uploadQueue.push({
        id: `text-${Date.now()}`,
        name: "Provided Source Material",
        type: "text",
        data: content
      });
    }
    ...
    // Boot background job asynchronously without blocking HTTP response
    setImmediate(() => {
      startBackgroundStudyJob(jobId).catch(err => {
        console.error(`Error in study material job ${jobId}:`, err);
      });
    });

    res.json({ success: true, jobId, message: "Study material PDF background generation started." });
  } catch (err: any) {
    console.error("Failed to start study job:", err);
    res.status(500).json({ error: err.message || "Failed to start background generation." });
  }
});

// List all study material background jobs for mentor
app.get("/api/mentor/study-jobs", requireAuth, async (req, res) => {
  try {
    const allJobs = await loadAllJobs();
    const user = (req as any).user;

    const studyJobs = Object.values(allJobs)
      .filter(j => j.jobType === "study_material")
      .sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime());

    res.json(studyJobs);
  } catch (err) {
    res.status(500).json({ error: "Failed to fetch study jobs" });
  }
});

// Get specific study material job status
app.get("/api/mentor/study-jobs/:jobId", requireAuth, async (req, res) => {
...
// Download/Stream generated study material PDF
app.get("/api/mentor/study-jobs/:jobId/pdf", async (req, res) => {
  const { jobId } = req.params;
  const reqLang = ((req.query.lang as string) || 'en').toLowerCase();
  const isBn = reqLang === 'bn' || reqLang.includes('bengali');

  try {
    const job = await loadJob(jobId);
    
    // Check specific lang file first e.g. jobId_bn.pdf or jobId_en.pdf
    let pdfPath = path.join(STORAGE_PDF_DIR, `${jobId}_${isBn ? 'bn' : 'en'}.pdf`);
    if (!fs.existsSync(pdfPath)) {
      pdfPath = getStudyPdfPath(jobId);
    }

    if (fs.existsSync(pdfPath)) {
      const rawFilename = isBn 
        ? (job?.pdfFilenameBn || `${job?.topic || 'Study_Guide'}_Bengali_Edition_MissionGrid.pdf`)
        : (job?.pdfFilename || `${job?.topic || 'Study_Guide'}_English_Edition_MissionGrid.pdf`);
    ...
    // Fallback: build buffer on the fly if generatedContent is present
    if (job && (job.generatedContent || job.generatedContentBn)) {
      const content = isBn ? (job.generatedContentBn || job.generatedContent) : (job.generatedContent || job.generatedContentBn);
      const edition = isBn ? 'bengali' : 'english';
      const pdfBuffer = await buildStudyMaterialPdfBuffer(content!, job.topic || "Study Material", edition);
      const rawFilename = isBn 
        ? (job.pdfFilenameBn || `${job.topic || 'Study_Guide'}_Bengali_Edition_MissionGrid.pdf`)
        : (job.pdfFilename || `${job.topic || 'Study_Guide'}_English_Edition_MissionGrid.pdf`);
    ...
    res.status(404).json({ error: "Generated PDF file not found" });
  } catch (err) {
...
// Cancel study job
app.post("/api/mentor/study-jobs/:jobId/cancel", requireMentor, async (req, res) => {
  const { jobId } = req.params;
  const job = await loadJob(jobId);
...
// Retry failed/cancelled study job
app.post("/api/mentor/study-jobs/:jobId/retry", requireMentor, async (req, res) => {
  const { jobId } = req.params;
  const job = await loadJob(jobId);
  if (!job) return res.status(404).json({ error: "Job not found" });
  ...
  const limitCheck = checkAIUsageAllowed(0.5);
  if (!limitCheck.allowed) {
    return res.status(400).json({ error: "Daily AI usage limit reached." });
  }

  job.status = 'queued';
  job.percent = 5;
  job.stage = 'Retrying pipeline';
  job.error = undefined;
  job.logs.push(`[${new Date().toLocaleTimeString()}] Retrying background PDF generation...`);
  await saveJob(jobId, job);
  ...
  res.json({ success: true, message: "Job re-queued for generation." });
});

// Delete study job
app.delete("/api/mentor/study-jobs/:jobId", requireMentor, async (req, res) => {
  const { jobId } = req.params;
  const job = await loadJob(jobId);
  if (job) {
    await deleteJob(jobId);
    const pdfPath = getStudyPdfPath(jobId);
    if (fs.existsSync(pdfPath)) {
...
app.post("/api/admin/premium/update", requireMentor, async (req, res) => {
  ...
  try {
    ...
    // 1.5 Automatically sync claims for the student on the backend
    let studentUid = studentData?.uid;
    if (!studentUid && studentId) {
      try {
        const db = getFirestore(getApp(), "(default)");
        const rolesSnap = await db.collection("user_roles").where("userId", "==", studentId).limit(1).get();
        if (!rolesSnap.empty) {
          studentUid = rolesSnap.docs[0].id;
        } else {
          // Fallback: check if the studentId is actually the uid
          const userDoc = await db.collection("users").doc(studentId).get();
          if (userDoc.exists) {
            studentUid = userDoc.data()?.uid || studentId;
          }
        }
      } catch (err) {
        console.warn("[Server] Error resolving studentUid for claims sync:", err);
      }
    }
    ...
    if (studentUid) {
      try {
        await syncCustomClaimsForUser(studentUid);
        console.log(`[Server] Automatically synchronized claims for student ${studentUid}`);
      } catch (claimsErr) {
        console.error(`[Server] Failed to sync custom claims for student ${studentUid}:`, claimsErr);
      }
    }
...
app.post("/api/admin/reset-device", requireMentor, async (req, res) => {
  ...
  try {
    ...
    let docId = studentId;
    if (!userDoc.exists) {
      // Find where mobile is equal to studentId
      const qSnap = await db.collection("users").where("mobile", "==", studentId).limit(1).get();
      if (!qSnap.empty) {
        docId = qSnap.docs[0].id;
      } else {
        return res.status(404).json({ error: `Student with ID or Mobile ${studentId} not found` });
      }
    }

    // Reset deviceId in both public and private collections
    await db.collection("users").doc(docId).set({ deviceId: "", androidDeviceId: "", desktopDeviceId: "" }, { merge: true });
    await db.collection("users_private").doc(docId).set({ deviceId: "", androidDeviceId: "", desktopDeviceId: "" }, { merge: true });

    console.log(`[Server] Device lock reset successfully for student document ID: ${docId}`);
    return res.json({ success: true, message: `Device reset successfully for student ID: ${docId}` });
  } catch (err: any) {
    console.error("[Server] Error resetting device ID:", err);
    return res.status(500).json({ error: "Failed to reset device ID", details: err.message });
  }
});
...
4. COMPLETE LEARNING ASSISTANT SOLUTION (MANDATORY): Every question MUST act as an expert teacher. The "explanation" field MUST contain a highly detailed, structured Markdown explanation following this EXACT structure:
   **1. Correct Answer**: (Display the correct option clearly).
   **2. Step-by-Step Explanation**: (Explain how the answer is obtained. Show every important calculation or logical step. Never jump directly to the final answer).
   **3. Concept Behind the Question**: (Explain the underlying concept or theory being tested. Teach the student why the answer is correct).
   **4. Why Other Options Are Wrong**: (Briefly explain why each incorrect option is wrong).
   **5. Shortcut / Exam Trick**: (Provide any faster approach, elimination technique, approximation, or exam shortcut if applicable).
   **6. Formula Section**: (If any formula is used, display it separately before solving).
   **7. Common Mistakes**: (Mention one or two mistakes students commonly make on similar questions).
   **8. Difficulty Level**: (Easy / Medium / Hard).
   **9. Related Concept**: (Suggest what topic the student should study next).
   
   Quality Rules for Solutions:
   - Language: Keep explanations simple, exam-oriented, and beginner-friendly. MUST NOT contain any Hindi. Use English or bilingual English and Bengali.
   - Print-Friendly: Do NOT copy headers or footers from the source document. Make the content printable friendly. Keep the explanation concise (single page maximum content).
   - Uniqueness: Never repeat the same answer twice, never duplicate paragraphs, never contradict yourself.
   - Completeness: Never output incomplete solutions. Always verify logical consistency.
   - Formatting: Use Markdown formatting with headings, bullet points, tables, and mathematical notation (using Unicode as specified below).
...
9. VISUAL EXPLANATION & DIAGRAMS (CRITICAL): Whenever the question involves geometry, maps, graphs, science, biology, reasoning, data interpretation, machines, flowcharts, seating arrangement, blood relation, clocks, calendars, number patterns, physics, chemistry, or any topic that benefits from visualization, generate a clean SVG diagram, table, flowchart, timeline, graph, or illustration inside the "diagram_svg" field. Never skip diagrams when they improve understanding. Ensure proper viewBox, high contrast lines, and visibility on both Light/Dark themes.
              ...
              questions: {
                ...
                items: {
                  ...
                  properties: {
                    ...
                    text: { type: Type.STRING, description: "Main text of the question in English. Include LaTeX equations where helpful, fix typos." },
                    text_bn: { type: Type.STRING, description: "Main text of the question translated to Bengali." },
                    options: {
                      type: Type.ARRAY,
                      items: { type: Type.STRING },
                      description: "For MCQ/MSQ, options array in English MUST contain exactly 4 items. For Boolean, provide exactly ['True', 'False']. For non-MCQ types like Integer, Subjective, Fill, etc., provide an empty array []."
                    },
                    options_bn: {
                      type: Type.ARRAY,
                      items: { type: Type.STRING },
                      description: "For MCQ/MSQ, options array translated to Bengali MUST contain exactly 4 items."
                    },
                    correctAnswers: {
                      type: Type.ARRAY,
                      items: { type: Type.STRING },
                    ...
                    explanation: { type: Type.STRING, description: "Detailed consolidated step-by-step explanation and solution in English." },
                    explanation_bn: { type: Type.STRING, description: "Detailed consolidated step-by-step explanation and solution translated to Bengali." },
                    stepwiseSolution: {
                      type: Type.ARRAY,
                      items: { type: Type.STRING },
                      description: "Detailed, step-by-step solution in English. Each string represents one properly aligned step."
                    },
                    stepwiseSolution_bn: {
                      type: Type.ARRAY,
                      items: { type: Type.STRING },
                      description: "Detailed, step-by-step solution translated to Bengali."
                    },
                    examApproach: { type: Type.STRING, description: "Exam approach, trick, or shortcut in English." },
                    examApproach_bn: { type: Type.STRING, description: "Exam approach, trick, or shortcut translated to Bengali." },
                    ruleOrTheorem: { type: Type.STRING, description: "The underlying concept, rule, or theorem in English." },
                    ruleOrTheorem_bn: { type: Type.STRING, description: "The underlying concept, rule, or theorem translated to Bengali." },
...
4. ANSWERS MUST HAVE 3 DISTINCT TYPES OF SOLUTIONS / EXPLANATIONS (Populate all three fields for EVERY question with deep educational content, NEVER leave them empty, generic, or copy-pasted. Each field MUST present a completely DIFFERENT approach to solving the question):
   - 'explanation': METHOD 1 - Detailed Step-by-Step Solution: Conventional, rigorous mathematical or logical step-by-step derivation leading to the correct answer. Show all intermediate steps and calculations clearly. MUST NOT contain any Hindi. Must be in English or bilingual English and Bengali.
   - 'examApproach': METHOD 2 - Exam Shortcut & Rapid Elimination Trick: Fastest exam approach, elimination trick, mental calculation, unit-digit/tens-digit check, back-substitution, or rapid approximation to solve the question in under 30 seconds. MUST NOT contain any Hindi. Must be in English or bilingual English and Bengali.
   - 'ruleOrTheorem': METHOD 3 - Core Theorem & Alternative Method: Alternative conceptual or formula-based approach, core theorem/identity derivation, visual method, or rule-based solution method. MUST NOT contain any Hindi. Must be in English or bilingual English and Bengali.
...
6. QUALITY AND COMPLETENESS:
   ...
   - VISUAL EXPLANATION & DIAGRAMS (CRITICAL): Whenever the question involves geometry, maps, graphs, science, biology, reasoning, data interpretation, machines, flowcharts, seating arrangement, blood relation, clocks, calendars, number patterns, physics, chemistry, or any topic that benefits from visualization, generate a clean SVG diagram, table, flowchart, timeline, graph, or illustration inside the "diagram_svg" field. Never skip diagrams when they improve understanding. Ensure proper viewBox, high contrast lines, and visibility on both Light/Dark themes.
            ...
            properties: {
              ...
              questions: {
                ...
                items: {
                  ...
                  properties: {
                    type: { type: Type.STRING, description: "MCQ, MSQ, Integer, Paragraph, Subjective, Boolean, or Fill." },
                    text: { type: Type.STRING, description: "Main body of the question in English. Fix spelling, OCR breaks, and list options perfectly." },
                    text_bn: { type: Type.STRING, description: "Main body of the question translated to Bengali. Must be provided." },
                    options: {
                      type: Type.ARRAY,
                      items: { type: Type.STRING },
                      description: "For MCQ/MSQ, options array in English MUST contain exactly 4 items. For Boolean, provide exactly ['True', 'False']. For non-MCQ types like Integer, Subjective, Fill, etc., provide an empty array []."
...
app.post("/api/auth/login", async (req, res) => {
  try {
    const { mobile, pin, role, verificationMethod, deviceId, deviceType } = req.body;
    if (!mobile || !pin || !role) {
    ...
    if (!isPasswordValid) {
      ...
      if (deviceType) {
        updates.lastDeviceType = deviceType;
      }
    ...
    if (role === 'mentor') {
        ...
        if (!validStudentRoles.includes(dbRole)) {
            return res.status(401).json({ error: "Authentication failed. Only students can log in here." });
        }
    }
    ...
    if (!isMagicLogin && deviceId && deviceId !== 'unknown' && role !== 'mentor') {
        const isDeviceLockExempted = sanitizedMobile === '9593126676';
        let androidDeviceId = privateData.androidDeviceId;
        let desktopDeviceId = privateData.desktopDeviceId;
        
        // Migrate legacy deviceId
        const legacyDeviceId = privateData.deviceId;
        if (legacyDeviceId && legacyDeviceId !== 'unknown' && !androidDeviceId && !desktopDeviceId) {
             const legacyType = privateData.lastDeviceType || (privateData.operatingSystem === 'Android' ? 'android' : 'desktop');
             if (legacyType === 'android') androidDeviceId = legacyDeviceId;
             else desktopDeviceId = legacyDeviceId;
        }

        const updateObj: any = {};
        
        // deviceType is passed from frontend ('android' or 'desktop')
        const currentDeviceType = deviceType === 'android' ? 'android' : 'desktop';

        if (currentDeviceType === 'android') {
            if (androidDeviceId && androidDeviceId !== deviceId && !isDeviceLockExempted) {
                return res.status(401).json({ error: "Device mismatch! This account is already registered to another Android device." });
            } else if (!androidDeviceId || isDeviceLockExempted) {
                updateObj.androidDeviceId = deviceId;
            }
        } else {
            if (desktopDeviceId && desktopDeviceId !== deviceId && !isDeviceLockExempted) {
                return res.status(401).json({ error: "Device mismatch! This account is already registered to another Desktop/Web device." });
            } else if (!desktopDeviceId || isDeviceLockExempted) {
                updateObj.desktopDeviceId = deviceId;
            }
        }

        if (Object.keys(updateObj).length > 0) {
            updateObj.lastDeviceType = currentDeviceType;
            await db.collection("users_private").doc(userId).set(updateObj, { merge: true });
            
            const publicUpdate: any = {};
            if (updateObj.androidDeviceId) publicUpdate.androidDeviceId = updateObj.androidDeviceId;
            if (updateObj.desktopDeviceId) publicUpdate.desktopDeviceId = updateObj.desktopDeviceId;
            if (Object.keys(publicUpdate).length > 0) {
                await db.collection("users").doc(userId).set(publicUpdate, { merge: true });
            }
        }
    }
...
// Initial call to set up Vite/Static assets at the end of the file
setupVite().then(() => {
  console.log("[Server] Vite/Static assets initialization completed.");
}).catch(err => {
  console.error("[Server] Critical error during Vite/Static setup:", err);
});
